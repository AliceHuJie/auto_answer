...
# 爬虫地址模板
search_movies_url = 'https://movie.douban.com/j/new_search_subjects?sort=U&range=0,10&tags=%E7%94%B5%E5%BD%B1&start={start}&countries=%E4%B8%AD%E5%9B%BD%E5%A4%A7%E9%99%86&year_range='+ year + ',' + year  # tags=电影 countries=大陆
...
# 连接本地mysql数据库
mysql_db = pymysql.connect(host="localhost", user="root", password='2736', db="auto_answer_for_movie", use_unicode=True,
                           charset="utf8")
mysql_cursor = mysql_db.cursor()

# 插入语句模板
insert_movie_crawer_manager_command = 'INSERT  IGNORE INTO auto_answer_for_movie.movie_crawer_manager (movie_id, movie_name, movie_url, cover_url, rate) VALUES ( %d, "%s", "%s", "%s", %.1f)'
insert_person_crawer_manager_command = 'INSERT IGNORE INTO auto_answer_for_movie.person_crawer_manager (person_id) VALUES (%d)'

insert_movie_command = 'INSERT IGNORE INTO `auto_answer_for_movie`.`movie`(movie_id, movie_title, movie_introduction, movie_rating, movie_release_date, movie_duration, movie_alias, movie_cover, movie_year, movie_language, movie_country, movie_bianju) ' \
                       'VALUES ({movie_id}, "{movie_title}", "{movie_introduction}", {movie_rating}, "{movie_release_date}", "{movie_duration}", "{movie_alias}", "{movie_cover}", {movie_year}),"{movie_language}","{movie_country}","{movie_bianju}";'
insert_person_command = 'insert IGNORE into `auto_answer_for_movie`.`person` (person_id, person_name, person_gender, person_english_name, person_biography, person_birth_place, person_birth_day, person_pic, person_introduction) ' \
                       'values ({person_id}, "{person_name}", "{person_gender}", "{person_english_name}", "{person_biography}", "{person_birth_place}", "{person_birth_day}", "{person_pic}", "{person_introduction}")'
insert_genre_command = 'insert IGNORE into genre (genre_id, genre_name) values (%s, %s)'
insert_country_command = 'insert IGNORE into country (country_id, country_name) values (%s, %s)'

insert_movie_to_director_command = 'INSERT IGNORE INTO `auto_answer_for_movie`.`movie_to_director`(`movie_id`,`director_id`)VALUES(%s, %s);'
insert_movie_to_actor_command = 'INSERT IGNORE INTO `auto_answer_for_movie`.`movie_to_actor`(`movie_id`,`actor_id`)VALUES(%s, %s);'
insert_movie_to_genre_command = 'INSERT IGNORE INTO `auto_answer_for_movie`.`movie_to_genre`(`movie_id`,`genre_name`)VALUES(%s, %s);'
insert_movie_to_country_command = 'INSERT IGNORE INTO `auto_answer_for_movie`.`movie_to_country`(`movie_id`,`country_name`)VALUES(%s, %s);'
insert_movie_to_language_command = 'INSERT IGNORE INTO `auto_answer_for_movie`.`movie_to_language`(`movie_id`,`language_name`)VALUES(%s, %s);'

mark_movie_crawled_command = 'UPDATE auto_answer_for_movie.movie_crawer_manager SET is_crawed = 1 WHERE movie_id={movie_id}'
mark_person_crawled_command =  'UPDATE auto_answer_for_movie.person_crawer_manager SET is_crawed = 1 WHERE person_id={person_id}'
...
def get_craw_movie_id_list(n):
    ...
    '''
    sql = 'SELECT movie_id FROM auto_answer_for_movie.movie_crawer_manager where is_crawed=0  limit %d' % n
...
def craw_movie_detail(movie_id):
    ...
    try:
        ...
        if u'编剧' in movie_info.keys():
            movie['movie_bianju'] = movie_info[u'编剧']
...
def insert_movie_to_country(movie):
    ...
    try:
        ...
        raise Exception('movie_to_country insert failed')
