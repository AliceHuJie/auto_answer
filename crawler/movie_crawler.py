...
# 这个暂时用不上，暂时只保存movie actor对应关系，最后再去爬演员信息
...
def craw_movie_and_insert(movie_id):
    ...
    try:
        ...
        # print 'movie_to_actor insert success'

        # movie_to_director 插入
        ...
        # print 'movie_to_director insert success'

        # movie_to_genre 插入 TODO 去掉了外键,因为不知道有哪些类型, 同时插入的是类型名
        genres = movie["genres"]
        ...
        print('movie_to_genre insert success')
        ...
        # movie_to_language 插入
        # TODO 暂时删除了语言外键，因为不知道有哪些语言, 同时插入的是语言名。 id 未知
        # languages = movie["languages"]
        # movie_to_language_list = [(movie_id, language_name) for language_name in languages]
        # mysql_cursor.executemany(insert_movie_to_language_command, movie_to_language_list)
        # print ('movie_to_language insert success')
...
def get_crawl_movie_list(start):
    url = ''
    insert_movie_crawer_manager_command
...
if __name__ == '__main__':
    ...
    #沒有country信息的
    movie_id_list = [1291858, 1291864, 1291876, 1293391]
    try:
